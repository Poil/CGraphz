function add_plugin_anchor(h2,cur_h3_array){
    if(h2!=null){
        ...
        for(h3_key in cur_h3_array) {
            ...
            if(h4_HTML=="") {
                h3_anchor=cur_h3_array[h3_key]['anchor'].replace(/\s/g,"%20");
                h3_HTML+='<li role="anchor"><a role="h3" tabindex="-1" href="#'+h3_anchor+'">'+cur_h3_array[h3_key]['name']+'</a></li>';
            } else {
                if(nb_h3>0) h3_HTML+='<li role="anchor" class="divider"></li>'; 
                h3_HTML+='<li role="anchor" class="dropdown-header">'+h3_key+'</li>'+h4_HTML;
            }
