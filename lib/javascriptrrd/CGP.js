var mouse_move = function (e) {
	if (this.rrdgraph.mousedown) {
		...
		setInputDate(this.rrdgraph.start, this.rrdgraph.end);
		try {
...
function setInputDate(timestart, timeend) {
	var myDate = new Date();
	myDate.setTime(timestart*1000);
	var myDateF=dateFormat(myDate,"yyyy-mm-dd HH:MM");
	$('#f_time_start').val(myDateF);
	myDate.setTime(timeend*1000);
	var myDateE=dateFormat(myDate,"yyyy-mm-dd HH:MM");
	$('#f_time_end').val(myDateE);
