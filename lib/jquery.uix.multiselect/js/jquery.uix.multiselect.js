...
;(function($, window, undefined) {
    ...
    $.widget("uix.multiselect", {
            ...
            this._elementWrapper = $('<div></div>').addClass('uix-multiselect ui-widget')
                ...
                .append(
                    $('<div></div>').addClass('multiselect-selected-list')
                        .append( $('<div></div>').addClass('ui-widget-header')
                            .append( btnDeselectAll = $('<button><span class="glyphicon glyphicon-step-forward"></span></button>', { type:"button" }).addClass('uix-control-right')
                ...
                ['right,top'.indexOf(this.options.availableListPosition)>=0?'prepend':'append'](
                    $('<div></div>').addClass('multiselect-available-list')
                        .append( $('<div></div>').addClass('ui-widget-header')
                            .append( btnSelectAll = $('<button><span class="glyphicon glyphicon-step-backward"></span></button>', { type:"button" }).addClass('uix-control-right')
        ...
        _initSearchable: function() {
            ...
            if (isToggle) {
                var that = this;
                this._buttons['search'] = $('<button><span class="glyphicon glyphicon-search"></span></button', { type:"button" }).addClass('uix-control-right')
        ...
        _resize: function() {
            ...
            this._elementWrapper.find('.multiselect-available-list')
                ...
                [tSize.toLowerCase()](hAv);  // account for borders
            ...
            this._elementWrapper.find('.multiselect-selected-list')
                ...
                [tSize.toLowerCase()](hSl); // account for borders
                //[tSize.toLowerCase()](this.element['outer'+tSize]() + 1); // account for borders
            ...
            this._lists['available'].height(hAv - 29);  // account for borders
            this._lists['selected'].height(hSl - 29);    // account for borders
        },
