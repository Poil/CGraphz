...
function refresh_graph(id,time_range,time_start,time_end) {
	...
	if (time_range!='') {
		var attrjs="Show_Popup($(this).attr('src').split('?')[1],'"+time_range+"','','')";
		$('#dashboard'+'> .imggraph').attr('onClick',attrjs);

	} else if (time_start!='' && time_end!='') {
		var attrjs="Show_Popup($(this).attr('src').split('?')[1],'','"+time_start+"','"+time_end+"')";
		$('#dashboard'+'> .imggraph').attr('onClick',attrjs);
	}
...
function Show_Popup(url,time_range,time_start,time_end) {
	...
	$('#popup').load('html/dashboard/zoom/d_zoom.php?'+url+'&graph_type=canvas&detail-width='+$('#dashboard').width()+'&detail-height='+$(window).height(), function() {
		$('#popup').fadeIn('fast');
