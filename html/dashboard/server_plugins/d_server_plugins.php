...
if (is_dir($RRD['rrdroot_dir']."/$cur_server->server_name/")) {
	...
	$tplugins = preg_find($myregex, $RRD['rrdroot_dir'].'/'.$cur_server->server_name, PREG_FIND_RECURSIVE|PREG_FIND_FULLPATH|PREG_FIND_SORTBASENAME);
	...
	$old_pi='';
	$old_subpg='';
	$myregex='#^('.$RRD['rrdroot_dir'].'/'.$cur_server->server_name.'/)(\w+)(?:\-(.*))?/(\w+)(?:\-(.*))?\.rrd#';
