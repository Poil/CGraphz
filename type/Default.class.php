...
class Type_Default {
	...
	function parse_ds_names($sources) {
		...
		foreach ($this->ds_names as $key=>$ds_name) {
			if (strstr($ds_name, '-')!==false) {
				$length = strlen(substr(strstr($ds_name, '-'), 1));
				$this -> ds_names[$key] = substr(strstr($ds_name, '-'), 1);
			} else if (strstr($ds_name, '.') !== false) {
				$length = strlen(strstr($ds_name, '.', true));
				$this -> ds_names[$key] = strstr($ds_name, '.', true);
			} else {
				$length = strlen((string)$ds_name);
			}
			if ($length > $max) {
				$max = $length;
			}
